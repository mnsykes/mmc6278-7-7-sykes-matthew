...
router
	...
	.post(checkAuth, async (req, res) => {
		...
		if (cartItem) {
			await db.query(`UPDATE cart SET quantity=quantity+? WHERE inventory_id=? AND user_id=?`, [
				quantity,
				inventoryId,
				req.session.userId
			]);
		} else {
			await db.query(`INSERT INTO cart(inventory_id, quantity, user_id) VALUES (?,?,?)`, [
				inventoryId,
				quantity,
				req.session.userId
			]);
...
router
	...
	.delete(checkAuth, async (req, res) => {
		const [{ affectedRows }] = await db.query(`DELETE FROM cart WHERE id=? AND user_id=?`, [
			req.params.cartId,
			req.session.userId
		]);
